...
sudo systemctl start mosquitto # start the service directly so we don't have to reboot
...
sudo systemctl start mqtt-exporter.service # start the service directly so we don't have to reboot
